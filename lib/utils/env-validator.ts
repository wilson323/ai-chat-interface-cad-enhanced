...
export function validateCriticalDependencies(): EnvironmentValidationResult {
  ...
  if ((REDIS_URL == null || REDIS_URL.trim() === '') || (REDIS_TOKEN == null || REDIS_TOKEN.trim() === '')) {
    checks.push({
  ...
  // 检查React版本兼容性
  try {
    type ReactLike = { version?: string };
    const reactPkg: ReactLike | null = (() => {
      try { return require('react') as ReactLike } catch { return null }
    })();
  ...
  try {
    ...
    const THREE: ThreeLike = (() => { try { return require('three') as ThreeLike } catch { return {} as ThreeLike } })();
    const threeVersion = THREE.REVISION != null ? `0.${THREE.REVISION}.0` : 'unknown';
  ...
  // 检查Next.js版本兼容性
  try {
    const nextPackage: { version?: string } = (() => { try { return require('next/package.json') as { version?: string } } catch { return { version: 'unknown' } } })();
    const nextVersion = nextPackage.version ?? 'unknown';
  ...
  // 检查UUID模块
  try {
    const uuidLike: { v4?: () => string } = (() => { try { return require('uuid') as { v4?: () => string } } catch { return {} as { v4?: () => string } } })();
    if (typeof uuidLike.v4 === 'function') {
      checks.push({
        name: 'UUID',
  ...
  // 检查CAD相关依赖
  try {
    const okIfc: unknown | null = (() => { try { return require('web-ifc') as unknown } catch { return null } })();
    const okIfcThree: unknown | null = (() => { try { return require('web-ifc-three') as unknown } catch { return null } })();
    if (okIfc == null || okIfcThree == null) { throw new Error('cad deps missing') }
    checks.push({
